...
의존성: 단일 `requirements.txt`에 코어(pymavlink·pydantic·numpy·rich)·ML(river)·개발
(pytest·ruff·mypy)을 담아 `pip install -r requirements.txt` 한 줄로 완주하며, 용량이 큰
perception(torch·ultralytics·opencv)만 선택 주석으로 분리한다.
